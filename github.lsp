...
(DEFUN TIRA_REPETIDO (L)
	(IF (NULL (CAR L)) ;SE PRIMEIRO ELEMENTO FOR NIL
		L	;RETORNA LISTA
		(IF (EQL(EXISTE (CDR L)(CAR L)) T) ;SE HOUVER O ELEMENTO NA LISTA
			(TIRA_REPETIDO (CDR L)) ;RECURSÃO PARA A FUNÇÃO FAZENDO NADA
			(CONS (CAR L)(TIRA_REPETIDO (CDR L))) ;SENÃO COLOCO O ELEMENTO NA FILA E FAÇO RECURSÃO
		)
	)
)
...
;RETIRA NILS DA FILA
(DEFUN TIRA_NIL (L)
	(IF (NULL (CDR L)) ;O RESTO DA FILA É NIL?
		(IF (NULL (CAR L)) ;O ELEMENTO SELECIONADO É NIL?
			(CONS NIL NIL)	;ACABA COM A FILA
			(CONS (CAR L) NIL) ;COLOCO ELEMENTO E ACABO A FILA
		)
		(IF (NULL (CAR L)) ;O ELEMENTO SELECIONADO É NIL?
			(TIRA_NIL (CDR L))	;RECURSÃO FAZENDO NADA
			(CONS (CAR L)(TIRA_NIL (CDR L))) ;COLOCO ELEMENTO NA FILA E FAÇO RECURSÃO
		)
	)
)

;RETORNA LISTA COM DISCIPLINAS SE EXISTIR ALUNO E PROFESSOR
		...
		(IF (NULL (CAAR BD)) ;existe disciplina?
			...
			(IF (NULL (CAADAR BD)) ;existe lista de aluno?
				(IF (NULL (CAR(CDADAR BD))) ;existe lista de professores?
					(DISCIPLINAS? (CDR BD))
					(TIRA_REPETIDO (TIRA_NIL (CONS (CAAR BD)(DISCIPLINAS? (CDR BD)))))	;se tudo existir, add disciplina na lista alem de tirar repetidos
				)	
				(TIRA_REPETIDO (TIRA_NIL (CONS (CAAR BD)(DISCIPLINAS? (CDR BD)))))
			)	 
		)
	)
)
...
(DEFUN INSERE_ALUNO (LISTA BD2)
	(IF (NULL (CDR LISTA)) ;AINDA TEM MAIS ELEMENTOS NA FILA?
		(CONS (CAR LISTA)(ALUNOS? (CDR BD2))) ;SE NÃO TIVER VOLTO PARA ALUNOS? E CONTINUO
		(CONS (CAR LISTA)(INSERE_ALUNO (CDR LISTA) BD2)) ;COLOCO O ELEMTENTO NA FILA E VEJO SE TEM MAIS
	)
)
	...
	(IF (NULL (CAR BD))
		...
		(IF (NULL (CAAR BD)) ;tem disciplina?
			...
			(IF (NULL(CAADAR BD)) ;tem lista de aluno?
				(ALUNOS? (CDR BD)) ;se não
				(TIRA_REPETIDO (TIRA_NIL (INSERE_ALUNO(CAADAR BD) BD))) ;tiro repetido da lista o qual insiro os alunos, retornando uma lista sem alunos repetidos
			)
		)
	)	
)
...
(DEFUN INSERE_PROF (LISTA BD2)
	(IF (NULL (CDR LISTA)) ;AINDA TEM MAIS ELEMENTOS NA FILA?
		(CONS (CAR LISTA)(PROFESSORES? (CDR BD2)))	;SE NÃO TIVER VOLTO PARA PROFESSORES? E CONTINUO
		(CONS (CAR LISTA)(INSERE_PROF (CDR LISTA) BD2))	;COLOCO O ELEMTENTO NA FILA E VEJO SE TEM MAIS
	)
)

(DEFUN PROFESSORES? (BD)
	(IF (NULL (CAR BD)) ;O PRIMEIRO ELEMENTO É NIL?
		NIL
		(IF (NULL (CAAR BD)) ;tem disciplina?
			(PROFESSORES? (CDR BD)) ;se não
			(IF (NULL(CAR(CDADAR BD))) ;tem lista de professor?
				(PROFESSORES? (CDR BD)) ;se não
				(TIRA_REPETIDO (TIRA_NIL (INSERE_PROF(CAR(CDADAR BD)) BD))) ;tiro repetido da lista o qual insiro os professores, retornando uma lista sem professores repetidos
			)
		)
	)	
)
